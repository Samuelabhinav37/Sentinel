...
## Phase 32 — MCP server: annotations, a composite context tool, coverage, vuln/SCA data

Asked to make the MCP server "more powerful and compact" and whether it needed more
tools. Answered by researching rather than guessing, given `mcp==2.0.0` and the Wazuh/
MISP endpoints involved were all either past this session's knowledge cutoff or never
exercised live in this project before: confirmed `mcp.types.ToolAnnotations`
(`read_only_hint`/`destructive_hint`/`idempotent_hint`/`open_world_hint`, passed via
`@mcp.tool(annotations=...)`) against the SDK's own docs; cross-checked Wazuh's
`GET /vulnerability/{agent_id}` and `GET /sca/{agent_id}` across two independent doc
sources (still can't hit a live Wazuh manager from here to go further, same ceiling as
every other tool in this file); confirmed MISP's `GET /events/view/{event_id}` is the
standard single-event lookup. One research dead end worth naming: an early fetched doc
example for the MCP client used a package called `httpx2`, unfamiliar enough to be a
plausible hallucination -- checked it against `mcp`'s actual `pyproject.toml` on GitHub
before trusting it (Phase 31), and it turned out to be real, `mcp`'s own HTTP backend.

**Five new tools**, all reusing existing auth helpers -- no new credentials, no new
external services:
- `get_alert_context(alert_id)` -- bundles the raw alert, both triage docs (the poll
  and push pipelines write distinct doc ids, `alert_id` and `alert_id-push`; both are
  tried, 404s swallowed rather than raised), and response actions in one call. Today's
  single-purpose tools meant an agent investigating one alert needed three round
  trips for data that was already sitting in three known places -- this collapses
  that into one, the main "more compact" win from this request. Deliberately does
  *not* try to resolve a Wazuh agent from `host.name`: Wazuh's agent filter-query
  syntax isn't something this project has verified against a live API, and guessing
  at it here would repeat the exact mistake this session has otherwise been careful
  to avoid (Velociraptor's stubs, the MISP endpoint checks above). The bundled alert
  already carries `host.name` for a caller to chain into `get_wazuh_agent_status`
  themselves.
- `get_detection_coverage()` -- exposes this repo's own ATT&CK coverage layer
  (`docs/attack_coverage.json`, already generated by `build_attack_navigator.py` and
  already committed) over MCP. Read from a **bind mount**
  (`infra/compose/mcp/docker-compose.yml`: `../../../docs:/app/docs:ro`), not baked
  into the image at build time -- matches how every other compose stack in this
  project already hands a container host data (mounted config/volumes, never `COPY`
  into the image for anything that changes independently of the code), and means the
  data is only as stale as this VM's last `git pull`, not the last image rebuild.
  This closes a real gap in Phase 31's detection advisor: it previously had no way to
  check whether a technique was already covered before proposing a new rule for it,
  so it could hallucinate a "gap" that was actually already handled.
- `get_agent_vulnerabilities(agent_id, severity=None)` / `get_agent_sca(agent_id)` --
  Wazuh's Vulnerability Detector and Security Configuration Assessment modules,
  reusing the existing JWT-cached `_wazuh_request` helper unchanged.
- `misp_get_event(event_id)` -- full MISP event context (attributes, tags, related
  objects) for deeper enrichment after `misp_search_ioc` finds a match, same
  `not_configured` guard and auth header the existing MISP tool already has.

**Tool annotations on all 15 tools.** Everything that only reads this project's own
infra is `read_only_hint=True, open_world_hint=False`; `misp_search_ioc`/
`misp_get_event` add `open_world_hint=True` (a real third party is on the other end);
`trigger_shuffle_response` is the only tool marked `destructive_hint=True` -- it's the
one tool with real effect (kills a process), and now says so structurally rather than
only in its docstring, so an MCP client's permission UI can auto-approve the read-only
tools and still gate the one that matters. `velociraptor_run_hunt` is annotated
action-but-non-destructive ahead of time, even while still stubbed/inert, so the
annotation doesn't need revisiting once it's wired up for real.

**First-class time-range params.** `search_alerts`/`search_index` gained optional
`from_time`/`to_time` (ISO8601), building a `bool`+`range` query instead of the plain
`query_string` shape when either is set -- additive, unchanged behavior when neither is
passed. `detection_advisor.py`'s lookback window moved off a hand-built Lucene
`@timestamp:[... TO *]` string and onto this, removing a fragile piece of string
construction now that the server supports it as a real parameter. It also now calls
`get_detection_coverage` once per run and tells the drafting prompt which techniques
are already covered, directly using the new tool to make Phase 31's deliberately
scoped, opportunistic gap-surfacing less error-prone without turning it into the full
systematic sweep that's still explicitly out of scope.

**Compaction.** The `urllib.request`/`urlopen`/`json.loads` sequence that was
duplicated across `_elastic_request`, `_wazuh_request`, and `misp_search_ioc` is now
one shared `_http_json(url, method, headers, body, timeout)` helper; each caller just
builds its own URL/headers/body. `trigger_shuffle_response` was deliberately **not**
folded into it -- Shuffle's webhook response isn't guaranteed to be JSON, and the
existing code already returns the raw status/body text rather than parsing it, so
routing it through a JSON-parsing helper would have been a real behavior change
disguised as a refactor.

**Verification ceiling, same as Phases 30-31:** `python3 -m py_compile` on both
changed scripts passed; `docker compose config` on the updated `infra/compose/mcp/
docker-compose.yml` passed with dummy env vars, confirming the new bind mount is
syntactically valid. No live Wazuh vulnerability/SCA response, MISP event, or
`get_alert_context` round trip has actually been exercised -- all of that stays a
human-run smoke test once the server is deployed, same as everything else in this
build.
...
Dockerfile — exposes 15 annotated tools over MCP's Streamable HTTP transport,
shared-secret-header auth: `search_alerts`/`search_index` (allowlisted patterns, both
with optional `from_time`/`to_time`), `get_triage`, `get_response_actions`, a
composite `get_alert_context` (alert + both triage docs + response actions in one
call), `trigger_shuffle_response` (the only tool annotated destructive),
`list_wazuh_agents`/`get_wazuh_agent_status`/`get_agent_vulnerabilities`/
`get_agent_sca` (Wazuh manager API, not the indexer, consistent with Phase 29),
`get_detection_coverage` (this repo's own ATT&CK layer, bind-mounted read-only from
the VM's repo checkout), `misp_search_ioc`/`misp_get_event` (report `not_configured`
until a real MISP URL/key is supplied), and `velociraptor_list_clients`/
`velociraptor_run_hunt` (deliberate stubs until a client is enrolled). `infra/compose/
velociraptor/` runs the official server image, config/PKI self-generated on first
boot. Built and `docker compose config`-validated on
`feature/mcp-server-dual-ai-triage`; not yet deployed to either live VM. See Phases
30 and 32.
...
`search_index` tool (now with a first-class `from_time` lookback param instead of a
hand-built Lucene range clause), asks an LLM (`claude-sonnet-5` default) whether the
fired rule needs refinement or the technique looks uncovered -- telling it up front,
via `get_detection_coverage`, which techniques already have a rule -- and writes any
proposal to `detections/drafts/` for manual promotion through the existing validate →
live-fire → `validation.yml` → deploy path. Never touches `detections/rules/`,
...
run it end-to-end. See Phases 31-32.
...
open from Phases 30-32: deploying `infra/compose/mcp/` and `infra/compose/velociraptor/`
...
real HTTP covering all 15 tools (including the new `get_alert_context`,
`get_detection_coverage`, `get_agent_vulnerabilities`/`get_agent_sca`, and
`misp_get_event`, none of which have been exercised against a live Elastic/Wazuh/MISP
instance yet), re-importing + live-testing the reworked `llm-triage-push.json`
(including a forced-disagreement case to confirm the Cross-Check Gate correctly
withholds automated response), and a real run of `detection_advisor.py` against live
review-queue data. A
